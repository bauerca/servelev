...
MapView.prototype = {
    ...
    evMouseWheel: function(ev, delta, deltaX, deltaY) {
        this.x += delta * 0.25 * this.canvas.width * this.scale;
        this.y -= delta * 0.25 * this.canvas.height * this.scale;
        this.scale *= delta > 0 ? 0.5 : 2;
        this.broadcast();
    },
    ...
    draw: function(data, nxData, nyData, bndsData, setter) {
        ...
        for (iyImg = 0; iyImg < nyImg; iyImg++) {
            ...
            for (ixImg = 0; ixImg < nxImg; ixImg++) {
                ...
                setter(imgData, 4 * (iyImg * nxImg + ixImg), v);
